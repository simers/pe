...
- **The SaaSpocalypse = the leverage-thesis trap** (see "Two ways to die from leverage"). ~1,200 of the 13,500 are software, bought in 2020–21 with debt that "assumed breakneck pandemic growth"; AI fears then slashed software multiples. A concentrated, levered thesis that reversed → owners now won't sell at the markdown → stuck. Same disease as TXU, in slow motion, across a whole vintage.
...
**The dark side — moats erode.** A moat is a snapshot, not a guarantee. The 2026 case is the **SaaSpocalypse** (liquidity-crunch deep dive): software "stickiness" that looked bulletproof in 2021 now faces AI tools that replicate the function cheaply. The question is never just "is there a moat?" but "**is it widening or eroding, and what could breach it?**" — a pre-mortem on the moat itself.
